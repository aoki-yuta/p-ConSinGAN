...
def train(opt):
    ...
    mask = functions.adjust_scales2image(mask_, opt)
    masks = functions.create_masks_pyramid(mask, opt)
    
    print("Training on image pyramid: {}".format([r.shape for r in reals]))
    print("Training on mask pyramid: {}".format([r.shape for r in masks]))#
    print("")
    ...
    for scale_num in range(opt.stop_scale+1):
        ...
        functions.save_image('{}/mask_scale.png'.format(opt.outf), masks[scale_num])
